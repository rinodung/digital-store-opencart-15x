...
class ModelMenuMegamenu extends Model { 
	...
	public function getDataIndex ( $id=null, $store_id = 0 ) {
		/*$data = array(); 
		$index = array();
		$query = $this->db->query("SELECT megamenu_id, parent_id FROM ".DB_PREFIX ."megamenu ORDER BY parent_id ASC" );
		$rows = $query->rows;
		foreach ($rows as $row) {
			$id = $row["megamenu_id"];
		...
		$parent = 1 ;
		$this->recursiveDelete( $parent, $store_id );
	 ...
	 * delete tree
	 */ 
	public function recursiveDelete( $parent, $store_id = 0 )
	{
		

		/* $reuslt = $this->getDataIndex();
		$data = $reuslt['data'];
		$index = $reuslt['index'];
		$parent_id = $parent_id === NULL ? "NULL" : $parent_id;
		if (isset($index[$parent_id])) {
			foreach ($index[$parent_id] as $id) {
				$this->delete_child_nodes($id);
				$sql = " DELETE FROM ".DB_PREFIX ."megamenu_description WHERE megamenu_id=".(int)$data[$id]["megamenu_id"] ;
				$this->db->query( $sql );

				$sql = " DELETE FROM ".DB_PREFIX ."megamenu WHERE megamenu_id=".(int)$data[$id]["megamenu_id"] . " AND store_id=" .(int)$store_id;
				$this->db->query( $sql );
			}
		}*/
		$sql = ' DELETE THANG HIEN TAI $parent ';
		if( $this->hasChild($parent) ){
			$data = $this->getNodes( $parent );
			foreach( $data as $menu ){
			
				if($menu['megamenu_id'] > 1) {
					 $this->recursiveDelete( $menu['megamenu_id'], $level+1, $store_id );
				}	
			}
		}
	}
	...
	public function delete( $id, $store_id = 0 ){
		...
		$sql = " DELETE FROM ".DB_PREFIX ."megamenu WHERE megamenu_id=".(int)$id . " AND store_id=" .(int)$store_id;
		$this->db->query( $sql );
		$sql = " DELETE FROM ".DB_PREFIX ."megamenu_description WHERE megamenu_id=".(int)$id;
		$this->db->query( $sql );

		$this->delete_child_nodes($id, $store_id);
